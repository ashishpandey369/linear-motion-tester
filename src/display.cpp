...
void Display::updateRunningTime(uint32_t totalSeconds, uint16_t millisPart)
{
    ...
    if (msChanged || ssChanged || mmChanged) {
        ...
        int16_t msBlockW = msValW + 2 + msUnitW;
        int16_t msFontH = LABEL_HEIGHT;
...
void Display::updateStatus(const String &status)
{
    ...
    tft.setFreeFont(nullptr);
    // Header status badge intentionally hidden. The main machine-status panel remains visible.
}
...
{
    // Top-right READY/RUNNING/STOPPED badge intentionally hidden.
    // Keep machineStatus and the main MACHINE STATUS panel unchanged.
}
